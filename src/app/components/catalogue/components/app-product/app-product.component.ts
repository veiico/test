...
export class AppProductComponent
  ...
  constructor(
    ...
    // this.currency = this.formSetting.payment_settings[0].symbol;
    if (!this.layout_type) {
      this.layout_type = 1;
    }
...
ngOnChanges() {
  ...
  setLangKeys() {
    ...
    if (
      this.cardInfo &&
      this.cardInfo.button_type &&
      this.cardInfo.button_type.button_names
    ) {
      this.addBtnTxt = this.cardInfo.button_type.button_names.add
        ? this.cardInfo.button_type.button_names.add
        : (this.languageStrings.add || 'Add')
      this.removeBtnTxt = this.cardInfo.button_type.button_names.remove
        ? this.cardInfo.button_type.button_names.remove
  ...
  getParamsByRoute() {
    this.routeSubsriber = this.route.params.subscribe(params => {
      ...
      if (productData && productData[this.categoryId]) {
        ...
        // this.store.dispatch({
        //   type: product.actionTypes.VENDOR_PRODUCT,
        //   payload: {
  ...
  getProducts(id) {
    // this.router.navigate(['/pages/app-products', id]);
  }
...
this.addCart(event.product, event.index);
  ...
  addSideOrder(products, index) {
    ...
    // } else {
    //   this.popup.showPopup('info', 2000, this.langJson['The selected quantity exceeds quantity available in stock.'], false);
    // }
  }

...
export class DataFieldsComponent implements OnInit {
  ...
  constructor(private sessionService: SessionService, private loader: LoaderService,
    private signupService: SignupService, private popup: PopupModalService, private ref: ChangeDetectorRef, private productTemplateService : ProductTemplateService) { }
  ...
  ngOnInit() {
    ...
    this.currency = this.sessionService.get('config').is_multi_currency_enabled ? this.currency : this.sessionService.get('config').payment_settings[0].symbol;
  }

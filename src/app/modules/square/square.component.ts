...
export class SquareComponent implements OnInit {
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.SQUARE) {
    ...
    if (message.length > 0) {
      if (this.post_enabled) {
        ...
        this.paymentMadeResponse.emit({
          'payment_method': PaymentMode.SQUARE,
          'status': 'error'
        });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.SQUARE);

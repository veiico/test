  ...
  ngOnInit() {
    ...
    window.onmessage =  (event) =>{
      ...
      if (typeof event.data == 'object') {
        if(event.data.payment_method == 4){
            if(event.data.status == 'success'){
              this.paypalResponse(event.data);
            }
            else{
              this.paypalResponse(event.data);
            }
...
// checks for ar translations
  ...
  initPaypal() {
    ...
    let data = {
      ...
      'payment_method': PaymentMode.PAYPAL,
    ...
    this.paymentService.getPaymentUrl(data).subscribe(response => {
      ...
      if (response.status === 200) {
        this.paypalPaymentUrl = this.domSanitizer.bypassSecurityTrustResourceUrl(response.data.transaction_url);
      } else {

...
export class AdyenComponent implements OnInit {
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.ADYEN) {
  ...
  setOnCloseWinListener(message) {
    ...
    if (message.length > 0) {
      ...
      this.paymentMadeResponse.emit({
        'payment_method': PaymentMode.ADYEN,
        'status': 'error'
      });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.ADYEN);

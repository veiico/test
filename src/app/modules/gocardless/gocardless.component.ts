...
@Component({
  ...
  templateUrl: './gocardless.component.html',
  styleUrls: ['./gocardless.component.scss']
})
export class GocardlessComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.GOCARDLESS) {
    ...
    if (message.length > 0) {
      if (this.post_enabled) {
        ...
        this.paymentMadeResponse.emit({
          'payment_method': PaymentMode.GOCARDLESS,
          'status': 'error'
        });
      }
    }
  ...
  getPaymentObject() {
    this.paymentService.getActivePaymentOption().subscribe(response => {
      this.paymentObj = response.data.filter(
        ele => ele.value == PaymentMode.GOCARDLESS
      );

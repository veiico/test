...
@Component({
  selector: 'app-truevo',
  templateUrl: './truevo.component.html',
  styleUrls: ['./truevo.component.scss']
})
export class TruevoComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.TRUEVO) {
    ...
    if (message.length > 0) {
      if (this.post_enabled) {
        ...
        this.paymentMadeResponse.emit({
          'payment_method': PaymentMode.TRUEVO,
          'status': 'error'
        });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.TRUEVO);

...
export class FlutterwaveComponent implements OnInit {
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.FLUTTERWAVE) {
      ...
      else if (event.data.data && event.data.data.status == 'successful') {
        let obj = {
          "txRef": event.data.transactionobject.txRef, "flwRef": event.data.transactionobject.flwRef, "redirectUrl": event.data.transactionobject.redirectUrl, "chargeResponseCode": event.data.transactionobject.chargeResponseCode, "status": event.data.transactionobject.status
        }

        let url = `${event.data.transactionobject.redirectUrl}?response=${encodeURIComponent(JSON.stringify(obj))}`

        this.sessionVar = window.open(url, '_blank')
      }
    }
  }
  ...
  setOnCloseWinListener(message) {
    ...
    if (message.length > 0) {
      ...
      this.paymentMadeResponse.emit({
        'payment_method': PaymentMode.FLUTTERWAVE,
        'status': 'error'
      });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.FLUTTERWAVE);

...
@Component({
  selector: 'app-paynet',
  templateUrl: './paynet.component.html',
  styleUrls: ['./paynet.component.scss']
})
export class PaynetComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.PAYNET) {
        ...
        this.paynetResponse(event.data);
  ...
  setOnCloseWinListener(message) {
    ...
    if (message.length > 0) {
      ...
      this.paymentMadeResponse.emit({
        'payment_method': PaymentMode.PAYNET,
        'status': 'error'
      });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.PAYNET);

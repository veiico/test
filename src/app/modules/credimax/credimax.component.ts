...
@Component({
  selector: 'app-credimax',
  templateUrl: './credimax.component.html',
  styleUrls: ['./credimax.component.scss']
})
export class CredimaxComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  ngOnInit() {
    ...
    //     ele => ele.value == PaymentMode.CREDIMAX
    //   );
    // });
    // this.post_enabled = this.paymentObj.payment_process_type == 1 ? true : false;
    // console.log(this.post_enabled)
  }
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.CREDIMAX) {
        ...
        this.credimaxResponse(event.data);
  ...
  setOnCloseWinListener() {
    this.paymentMadeResponse.emit({
      'payment_method': PaymentMode.CREDIMAX,
      'status': 'error'
    });
    this.showBackground = false;
  };
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.CREDIMAX);

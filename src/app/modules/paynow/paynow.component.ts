...
export class PaynowComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        window.onmessage = (event) => {
            ...
            if (typeof event.data == 'object') {
                if (event.data.payment_method == PaymentMode.PAYNOW) {
                    if (event.data.status == 'success') {
                        this.paynowResponse(event.data);
                    }
                    else {
                      setTimeout(() => {
    ...
    initPaynow() {
        ...
        let data = {
            ...
            payment_method: PaymentMode.PAYNOW,
            app_type: 'WEB',
        ...
        this.paynowService.getPaynowLink(data).subscribe(response => {

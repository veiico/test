...
@Component({
  selector: 'app-gtbank',
  templateUrl: './gtbank.component.html',
  styleUrls: ['./gtbank.component.scss']
})
export class GtbankComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      ...
      if (event.data.payment_method == PaymentMode.GTBANK) {
  ...
  setOnCloseWinListener(message) {
    ...
    if (this.dataReceived && this.dataReceived.status == 'success') {
      ...
      this.paymentMadeResponse.emit({
        'payment_method': PaymentMode.GTBANK,
        'status': 'error'
      });
      // this.showBackground = false;
    }
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.GTBANK);

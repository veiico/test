...
@Component({
  selector: 'app-paywayone',
  templateUrl: './paywayone.component.html',
  styleUrls: ['./paywayone.component.scss']
})
export class PaywayoneComponent implements OnInit {
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      ...
      if (event.data.payment_method == PaymentMode.PAYWAYONE) {
  ...
  setOnCloseWinListener(message) {
    ...
    if (this.dataReceived && this.dataReceived.status == 'success') {
      ...
      this.paymentMadeResponse.emit({
        'payment_method': PaymentMode.PAYWAYONE,
        'status': 'error'
      });
      // this.showBackground = false;
    }
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.PAYWAYONE);

...
@Component({
  selector: 'app-etisalat',
  templateUrl: './etisalat.component.html',
  styleUrls: ['./etisalat.component.scss']
})
export class EtisalatComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.ETISALAT) {
    ...
    if (message.length > 0) {
      if (this.post_enabled) {
        ...
        this.paymentMadeResponse.emit({
          'payment_method': PaymentMode.ETISALAT,
          'status': 'error'
        });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.ETISALAT);

...
@Component({
  selector: 'app-nmi',
  templateUrl: './nmi.component.html',
  styleUrls: ['./nmi.component.scss']
})
export class NmiComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.NMI) {
  ...
  setOnCloseWinListener(message) {
    ...
    if (message.length > 0) {
      ...
      this.paymentMadeResponse.emit({
        'payment_method': PaymentMode.NMI,
        'status': 'error'
      });
    }
    else {
  ...
  getPaymentObject() {
    if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
      this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == PaymentMode.NMI);

...
@Component({
  selector: 'app-suncash',
  templateUrl: './suncash.component.html',
  styleUrls: ['./suncash.component.scss']
})
export class SuncashComponent implements OnInit {
  languageStrings: any={};
  public config: any;
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == PaymentMode.SUNCASH) {
  ...
  setOnCloseWinListener(message) {
    ...
    if (message.length > 0) {
      ...
      else if(this.paymentFor == 2) {
        this.paymentMadeResponse.emit({
          'payment_method': PaymentMode.SUNCASH,
          'status': 'error'
        });
      }
    }
  ...
  getPaymentObject() {
    this.paymentService.getActivePaymentOption().subscribe(response => {
      this.paymentObj = response.data.filter(
        ele => ele.value == PaymentMode.SUNCASH
      );

...
export class PayuComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        window.onmessage = (event) => {
            if (typeof event.data == 'object') {
                if (event.data.payment_method == PaymentMode.PAYU) {
                    if (event.data.status == 'success') {
                        this.payuResponse(event.data);
                    }
                    else {
                        this.payuResponse(event.data);
                    }
    ...
    ngOnDestroy() {
        ...
        clearInterval(this.closeWindowListener);
    }
    ...
    initPayu() {
        ...
        // this.paymentWinRef.document.title = 'Payment Process';
        // this.paymentWinRef.document.body.innerHTML = "<h5>Don't close or refresh the payment window .....</h5>";
        ...
        //     payment_method: PaymentMode.PAYU,
        //     app_type: 'WEB',
        ...
        //     payment_for : this._triggerPayment.payment_for ? this._triggerPayment.payment_for :undefined
        ...
        this.payuService.getPayuLink(data).subscribe(response => {
            if (response.status === 200) {
                this.loader.hide();
                this.setOnCloseWinListener();
                this.sessionService.paymentWinRef.location.href = response.data.transaction_url;
    ...
    setOnCloseWinListener() {
        this.closeWindowListener = setInterval(() => {
            if (this.sessionService.paymentWinRef.closed) {
                this.paymentMadeResponse.emit({
                    'payment_method': PaymentMode.PAYU,
                    'status': 'error'
                });

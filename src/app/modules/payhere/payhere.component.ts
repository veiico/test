...
export class PayhereComponent implements OnInit {
  ...
  @Input() set triggerPayment(val: any) {
    if (val) {
      ...
      if (this.loginResponse && this.loginResponse.formSettings[0].activePaymentMethods) {
        this.paymentObj = this.loginResponse.formSettings[0].activePaymentMethods.filter(ele => ele.value == this.paymentMode.PAYHERE);
  ...
  constructor(public loader: LoaderService, protected sessionService: SessionService, public payhereService: PayhereService, protected popup: PopUpService, protected cdRef: ChangeDetectorRef, protected appService: AppService) {
  ...
  initPayHere() {
    ...
    let data = {
      ...
      'payment_method': PaymentMode.PAYHERE,
    ...
    this.post_enabled = this.config.post_payment_enable;
   ...
   if(this.isSourceCustom && this.config.onboarding_business_type === OnboardingBusinessType.LAUNDRY && this.sessionService.getByKey('app', 'payment') && this.sessionService.getByKey('app', 'payment').remaining_balance && this.sessionService.getByKey('app', 'payment').remaining_balance > 0){
    ...
    // if (data.currency === 'USD' || data.currency === 'LKR') {
    this.payhereService.getPayHereLink(data).subscribe(response => {
      this.loader.hide();
  ...
  private onWindowMessage(event: any) {
    if (typeof event.data == 'object') {
      if (event.data.payment_method == 4194304) {
        ...
        this.payHereResponse(event.data);
        this.cdRef.detectChanges();
      }
    }
  }
